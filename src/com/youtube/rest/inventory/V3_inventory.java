...
public class V3_inventory{
	 ...
	 * example of using JSONArray and JSONObject. Note: This is apart of the
	 * Jersey version 1, but I'm not sure if of the version 2 Jersey.
	 */
	@POST
	...
	public Response addPcParts2(String incomingData) throws Exception{
		...
		try{
			...
			// looks for THAT key
			int http_code = db.insertIntoPC_PARTS(partsData.optString("PC_PARTS_TITLE"),
	...
	/**
	 * This method will allow you to update data in the PC_PARTS table. In this
	 * example we are using both PathParms and the message body (payload).
	 * 
	 * @param brand
	 * @param item_number
	 * @param incomingData
	...
	@Consumes({MediaType.APPLICATION_FORM_URLENCODED, MediaType.APPLICATION_JSON})
	@Produces(MediaType.APPLICATION_JSON)
	public Response updateItem(@PathParam("brand") String brand, @PathParam("item_number") int item_number,
		...
		try{
			...
			JSONObject partsData = new JSONObject(incomingData); // we are using
																	// json
																	// objects
																	// to parse
																	// data
			...
			http_code = db.updatePC_PARTS(pk, avail);
	 ...
	 * 
	 * We really only need the primary key from the message body but I kept the
	 * same URL path as the update (PUT) to let you see that we can use the same
	...
	@Consumes({MediaType.APPLICATION_FORM_URLENCODED, MediaType.APPLICATION_JSON})
	@Produces(MediaType.APPLICATION_JSON)
	public Response deleteItem(@PathParam("brand") String brand, @PathParam("item_number") int item_number,
			String incomingData) throws Exception{
		...
		try{
			...
			http_code = db.deletePC_PARTS(pk);
